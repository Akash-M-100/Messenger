...
export class TwilioVoiceProvider implements IChannelProvider {
  readonly providerId = "twilio_voice";
  ...
  async send(payload: SendPayload): Promise<SendResult> {
    ...
    const twiml = `<Response><Say>${payload.content?.body || "Hello"}</Say></Response>`;
    ...
    return {
      ...
      cost: result.price ? Math.abs(parseFloat(result.price)) : 0.02,
    };
  }

  async getStatus(messageId: string) {
    const response = await fetch(
      `https://api.twilio.com/2010-04-01/Accounts/${this.accountSid}/Calls/${messageId}.json`,
      {
        headers: {
          Authorization: this.getAuthHeader(),
        },
