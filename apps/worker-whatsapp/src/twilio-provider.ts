...
export class TwilioWhatsAppProvider implements IChannelProvider {
  readonly providerId = "twilio_whatsapp";
  ...
  async send(payload: SendPayload): Promise<SendResult> {
    const response = await fetch(
      `https://api.twilio.com/2010-04-01/Accounts/${this.accountSid}/Messages.json`,
      {
        ...
        body: new URLSearchParams({
          ...
          To: `whatsapp:${payload.recipient.phone || ""}`,
    ...
    return {
      ...
      cost: result.price ? Math.abs(parseFloat(result.price)) : 0.01,
    };
  }
